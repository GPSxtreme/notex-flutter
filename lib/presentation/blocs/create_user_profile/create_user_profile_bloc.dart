...
class CreateUserProfileBloc extends Bloc<CreateUserProfileEvent, CreateUserProfileState> {
  CreateUserProfileBloc() : super(CreateUserProfileInitial()) {
    on<CreateUserProfileOpenDatePickerEvent>((event, emit) => emit(CreateUserProfileOpenDatePickerState()));
    on<CreateUserProfileCreateEvent>(handleUserProfileCreate);
  }

  FutureOr<void> handleUserProfileCreate(CreateUserProfileCreateEvent event,emit)async{
    // upload profile picture
    try{
      emit(CreateUserProfileLoadingState());
      await UserRepository.updateUserProfilePicture(event.imageFile).then(
          (_)async{
            // update remaining data
            await UserRepository.updateUserData(event.data).then(
                (response) {
                  if(response.success){
